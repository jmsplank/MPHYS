...
fig = plt.figure(figsize=[12.2, 16.3])
...
for i, month in enumerate(months):
    with Timer('Loading IMF for {}'.format(month)):
        ...
        imf_temp.bx = imf_temp.bx.mask(imf_temp.bx > 1000).interpolate().astype('float')
        imf_temp.by = imf_temp.by.mask(imf_temp.by > 1000).interpolate().astype('float')
        imf_temp.bz = imf_temp.bz.mask(imf_temp.bz > 1000).interpolate().astype('float')
        imf_temp.dropna()
        ...
        ax.set_title(month)
        ax.hist2d(imf_temp.by, imf_temp.bz, 100, norm=mpl.colors.LogNorm(),
                  cmap='viridis', range=[[-40, 40], [-40, 40]])
        ax.grid(alpha=0.6)
